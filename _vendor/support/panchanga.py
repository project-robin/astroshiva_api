...
def sunrise(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(float(jd - tz/24), int(swe.SUN), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_RISE))
...
def sunset(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(float(jd - tz/24), int(swe.SUN), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_SET))
...
def moonrise(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(float(jd - tz/24), int(swe.MOON), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_RISE))
  rise = result[1][0]  # julian-day number
  # Convert to local time
...
def moonset(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(float(jd - tz/24), int(swe.MOON), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_SET))
  setting = result[1][0]  # julian-day number
  # Convert to local time
...
def gauri_chogadiya(jd, place):
  lat, lon, tz = place
  tz = place.timezone
  srise = swe.rise_trans(float(jd - tz/24), int(swe.SUN), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_RISE))[1][0]
  sset = swe.rise_trans(float(jd - tz/24), int(swe.SUN), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_SET))[1][0]
  day_dur = (sset - srise)
  ...
  srise = swe.rise_trans(float((jd + 1) - tz/24), int(swe.SUN), float(lon), float(lat), 0.0, 0.0, 0.0, int(_rise_flags + swe.CALC_RISE))[1][0]
